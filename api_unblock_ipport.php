	...
	foreach ($ips as $ipItem) {
		foreach ($ports as $portItem) {
			...
			if ($target_server) {
				...
				$msg = "unblock_ipport {$ipItem} {$portItem} @server={$target_server}";
				...
				$msg = "unblock_ipport {$ipItem} {$portItem} @servers={$target_servers}";
			} elseif ($target_group) {
				$r->srem("fw:block:ipports:group:{$target_group}", [$ipport]);
				$msg = "unblock_ipport {$ipItem} {$portItem} @group={$target_group}";
				...
				$msg = "unblock_ipport {$ipItem} {$portItem} @groups={$target_groups}";
			} else {
				$r->srem('fw:block:ipports', [$ipport]);
				$msg = "unblock_ipport {$ipItem} {$portItem}";
			}
