	...
	foreach ($ips as $ipItem) {
		foreach ($ports as $portItem) {
			...
			if ($target_server) {
				...
				$msg = "unallow_ipport {$ipItem} {$portItem} @server={$target_server}";
				...
				$msg = "unallow_ipport {$ipItem} {$portItem} @servers={$target_servers}";
			} elseif ($target_group) {
				$r->srem("fw:allow:ipports:group:{$target_group}", [$ipport]);
				$msg = "unallow_ipport {$ipItem} {$portItem} @group={$target_group}";
				...
				$msg = "unallow_ipport {$ipItem} {$portItem} @groups={$target_groups}";
			} else {
				$r->srem('fw:allow:ipports', [$ipport]);
				$msg = "unallow_ipport {$ipItem} {$portItem}";
			}
