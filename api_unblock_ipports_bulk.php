...
try {
    ...
    foreach ($clean as $e) {
        ...
        if ($e['target_type'] === 'server') {
            $r->srem('fw:block:ipports:server:' . $e['target_value'], [$ipport]);
            $msg = "unblock_ipport {$ip} {$port} @server={$e['target_value']}";
        } elseif ($e['target_type'] === 'group') {
            $r->srem('fw:block:ipports:group:' . $e['target_value'], [$ipport]);
            $msg = "unblock_ipport {$ip} {$port} @group={$e['target_value']}";
        } else {
