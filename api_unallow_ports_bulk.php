...
// entries: [{ port, target_type, target_value }]
...
try {
    ...
    foreach ($clean as $e) {
        ...
        if ($e['target_type']==='server') {
            $r->srem('fw:allow:ports:server:'.$e['target_value'], [$portRaw]);
            $msg = "unallow_port {$portRaw} @server={$e['target_value']}";
        } elseif ($e['target_type']==='group') {
            $r->srem('fw:allow:ports:group:'.$e['target_value'], [$portRaw]);
            $msg = "unallow_port {$portRaw} @group={$e['target_value']}";
        } else {
